    ...
    I2C: HalI2c,
{
    pub fn new_frame(&mut self) {
        if let Some(vram) = self.vram.as_mut() {
            for i in 0..(self.row / OLED_PAGE) as usize {
...
impl<'a, I2C: HalI2c> SSD1306_V2<'a, I2C> {
    ...
    pub fn show_frame(&mut self) {
        let ptr = self.vram.ram.as_ptr();
        let row_len = (self.col + 1) as usize;
        for i in 0..(self.row / OLED_PAGE as u16) as usize {
            self.send_cmd(0xB0 + i as u8);
            self.send_cmd(0x02);
            self.send_cmd(0x10);
            let start = i * row_len;
            let data = unsafe { slice::from_raw_parts(ptr.add(start), row_len) };
            self.send(data);
        }
    }
}
